...
Shader "Hidden/MicroSplat/MicroSplat_Base331959158" {
   Properties {
...
_TerrainNormalmapTexture("InstancedPerPixelNormal", 2D) = "bump" {}

   }
   SubShader {
      Tags{"RenderPipeline"="LightweightPipeline" "RenderType" = "Opaque" "Queue" = "Geometry+100" "IgnoreProjector" = "False" "SplatCount" = "4"}
...
void TerrainInstancing(inout float4 vertex, inout float3 normal, inout float2 uv)
{
    ...
    float height = UnpackHeightmap(SAMPLE_TEXTURE2D_LOD(_TerrainHeightmapTexture, sampler_TerrainNormalmapTexture, sampleUV, 0));
...
GraphVertexInput ApplyMeshModification(GraphVertexInput input)
{
   #if !_MICROMESH
    ...
    return input;
      ...
      half3 ComputeSSS(float3 L, float3 V, float3 N, half3 lightColor, 
         half3 tint, half thickness, half distortion, half scale, half power)
      {
      ...
      #undef MICROSPLAT_SAMPLE_TEX2D_GRAD
      ...
      #define MICROSPLAT_SAMPLE_TEX2D_GRAD(tex,coord,dx,dy)                 SAMPLE_TEXTURE2D_GRAD(tex,sampler_##tex,coord,dx,dy)
      ...
      void SampleSpecular(Config config, TriplanarConfig tc, inout RawSamples s, MIPFORMAT mipLevel)
      {
         #if _DISABLESPLATMAPS
            return;
         #endif
               ...
               #endif
               {
                  half4 a0 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv0[0], config.cluster0, d0);
                  half4 a1 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv0[1], config.cluster0, d1);
                  half4 a2 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv0[2], config.cluster0, d2);
                  #if _PERTEXTRIPLANAR
                  if (pttri0.x > 0.66)
                  {
                     a0 = a1;
               ...
               {
                  half4 a0 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv1[0], config.cluster1, d0);
                  half4 a1 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv1[1], config.cluster1, d1);
                  half4 a2 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv1[2], config.cluster1, d2);
                  #if _PERTEXTRIPLANAR
                  if (pttri1.x > 0.66)
                  {
                     a0 = a1;
               ...
               #if !_MAX2LAYER
               {
                  half4 a0 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv2[0], config.cluster2, d0);
                  half4 a1 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv2[1], config.cluster2, d1);
                  half4 a2 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv2[2], config.cluster2, d2);
                  #if _PERTEXTRIPLANAR
                  if (pttri2.x > 0.66)
                  {
                     a0 = a1;
               ...
               #if !_MAX3LAYER || !_MAX2LAYER
               {
                  half4 a0 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv3[0], config.cluster3, d0);
                  half4 a1 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv3[1], config.cluster3, d1);
                  half4 a2 = MICROSPLAT_SAMPLE_SPECULAR(tc.uv3[2], config.cluster3, d2);
                  #if _PERTEXTRIPLANAR
                  if (pttri3.x > 0.66)
                  {
                     a0 = a1;
      ...
      MicroSplatLayer Sample(Input i, half4 weights, inout Config config, float camDist, float3 worldNormalVertex)
      {
         ...
         #if _USESPECULARWORKFLOW
            SampleSpecular(config, tc, samples, specLOD);
         #endif
   ...
   // ADVANCEDTERRAIN_ENTRYPOINT 
         ...
         #if _PERTEXSSS || _MESHCOMBINEDUSESSS || (_SNOW && _SNOWSSS)
         {
               ...
               float3 lightDir = _gGlitterLight.xyz;
            #else
            ...
            o.Emission += ComputeSSS(lightDir, worldView, WorldNormalVector(i, half3(normSAO.xy, 1)),
               lightColor, SSSTint, SSSThickness, _SSSDistance, _SSSScale, _SSSPower);
         }
         #endif
    ...
    MicroSplatLayer DoMicroSplat(inout SurfaceDescriptionInputs IN)
    {
       ...
       float3 worldNormalVertex = IN.WorldSpaceNormal;
        ...
        #if defined(UNITY_INSTANCING_ENABLED) || _PERPIXNORMAL               
           ...
           l.Normal = l.Normal.xzy;
            ...
            l.Metallic = 0;
            l.Occlusion = 1;
            l.Emission = half3(0,0,0);
            l.Alpha = 1;
        #elif _DEBUG_OUTPUT_NORMAL
            l.Albedo = float3(l.Normal.xy * 0.5 + 0.5, l.Normal.z);
            l.Normal = float3(0,0,1);
            l.Smoothness = 0;
            l.Metallic = 0;
            l.Occlusion = 1;
            ...
            l.Emission = half3(0,0,0);
            l.Alpha = 1;
        #elif _DEBUG_OUTPUT_SPECULAR
            l.Albedo = l.Specular.xxx;
            l.Normal = float3(0,0,1);
            l.Smoothness = 0;
            l.Metallic = 0;
            l.Occlusion = 1;
